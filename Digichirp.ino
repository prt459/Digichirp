// Digi-chirp!
// Basic CW memory keyer and beacon
//  - option to superimpose digitally simulated chirp and drift, for old times' sake. 
//  - option to interface with an si5351 for a CW transmitter, signal generator or beacon. 
//
// VK3HN, Apr 2017 to Nov 2022.
...
#define CW_IDENT
#define CW_IDENT_SECS     5  // seconds between CW ident 
#define CW_IDENT_WPM      60  // CW ident speed (dot length mS)

//unsigned long freq = 353600000;
unsigned long   freq = 703200000;

unsigned long drift=0;
unsigned long freq_start = freq; 
...
//----CHIRP----------------------------------------------------------------------------
void chirp(int ms_durn)
{
  unsigned long freq_base = freq;
  unsigned long ms_entry = millis(); 
  unsigned long chirp_delta;  

#define CHIRP_MS        5  // mSec interval at which chirp increment is applied (typical range 2 rapid chirp to 150 slow/discrete chirp)
#define CHIRP_UP    180  // increment for each downward chirp movement (typical range 0 no chirp to 100 heavy chirp)
//#define CHIRP_DOWN   -150  // increment for each upward chirp movement (typical range 0 no chirp to -100 heavy chirp)
//#define CHIRP_RANDOM    0  // random chirp in both directions  (value not used)
#define CHIRP_MULT     15  // random chirp multiplier  (typical range 0 no chirp to 20 widely deviating chirp)

#define DRIFT_MS       20  // mSec interval at which drift increment is applied (typical range 0 no drift to 100 rapid/continuous drift)
#define DRIFT_UP      100  // increment for each upward drift movement (typical range 0 no drift to 100 large drift increments)
// #define DRIFT_DOWN -50  // increment for each downward drift movement (as above)

  unsigned int c = 0; 

  while( (abs(millis() - ms_entry) < ms_durn) )
  {
//    freq = freq + CHIRP_INCR_DOWN;
    ...
    drift = DRIFT_DOWN;
#endif
    if(millis()%DRIFT_MS == 0)  {
      freq = freq + drift;    
      c++;
    }

    // chirp
#if defined(CHIRP_DOWN)
    chirp_delta = CHIRP_DOWN;
#endif
#if defined(CHIRP_UP)
    chirp_delta = CHIRP_UP;
#endif
#if defined(CHIRP_RANDOM)
    chirp_delta = CHIRP_MULT*random(CHIRP_DOWN, CHIRP_UP);
#endif

   if(millis()%CHIRP_MS == 0)  freq = freq + chirp_delta;

    si5351.set_freq( freq, SI5351_CLK0); 
  }
  freq = freq_base + (c * drift);
}
//-------------------------------------------------------------------------------------
...
//  si5351.set_freq(freq * SI5351_FREQ_MULT, SI5351_CLK0);  
  ...
  Serial.print(".");
  chirp(dot_length_ms);  // key down for one dot period
  noTone(PIN_TONE_OUT);
...
//  si5351.set_freq(freq * SI5351_FREQ_MULT, SI5351_CLK0);  
  ...
  if(ch_counter % SERIAL_LINE_WIDTH == 0) Serial.println(); 
  Serial.print("-");
  chirp(dot_length_ms * CW_DASH_LEN);  // key down for CW_DASH_LEN dot periods
  noTone(PIN_TONE_OUT);
...
{
  Serial.begin(9600);  
  Serial.println("\nBasic CW keyer and beacon VK3HN 26-Nov-2021, please reuse, hack and pass on.");
...
{  
  ...
  if(ident_secs_count == CW_IDENT_SECS)
  {
    ...
    {
      freq_start = freq;
      play_message("( V V V DE VK3HN )", CW_IDENT_WPM);  // only Id if not transmitting
    }
    ident_secs_count = 0;
  }
#endif 
